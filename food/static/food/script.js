...
function displayResults(places, typeLabel) {
  ...
  // To match the 'Stay' app, we'll keep the panel clean
  ...
  const statusElem = document.getElementById("status");
  if (statusElem) {
    statusElem.innerText = `Found ${displayCount} places near you`;
  }
  ...
  places.slice(0, limit).forEach((place, index) => {
    ...
    let image = foodImgs[index % foodImgs.length];
    let fallbackImage = foodImgs[0];

    if (typeLabel === "Bus Stop") {
      image = busImgs[index % busImgs.length];
      fallbackImage = busImgs[0];
    } else if (typeLabel === "Railway Station") {
